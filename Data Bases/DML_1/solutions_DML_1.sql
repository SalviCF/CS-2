...
-- 10.
SELECT NOMBRE, APELLIDO1, APELLIDO2, FECHA_NACIMIENTO, TRUNC((MONTHS_BETWEEN(CONVERT(DATE, GETDATE()),
                                                                              FECHA_NACIMIENTO)) /12) AS EDAD
FROM PROFESORES
WHERE TRUNC((MONTHS_BETWEEN(CONVERT(DATE, GETDATE()), FECHA_NACIMIENTO))/12) < 30;

-- 11.
SELECT UPPER(NOMBRE), UPPER(APELLIDO1), UPPER(APELLIDO2), TRUNC(MONTHS_BETWEEN(CONVERT(DATE, GETDATE()),
                                                                                        ANTIGUEDAD) /12/3) AS TRIENIOS_DOCENCIA
FROM PROFESORES
WHERE TRUNC(MONTHS_BETWEEN(CONVERT(DATE, GETDATE()), ANTIGUEDAD)/12/3) > 3;
...
-- 14.
SELECT DNI, NOMBRE, APELLIDO1, APELLIDO2, FECHA_PRIM_MATRICULA,  TRUNC(MONTHS_BETWEEN(CONVERT(DATE, GETDATE()),
                                                                                      FECHA_PRIM_MATRICULA)) AS MESES_MATRICULADO
FROM ALUMNOS
WHERE (MONTHS_BETWEEN(CONVERT(DATE, GETDATE()), FECHA_PRIM_MATRICULA)) < 2;
