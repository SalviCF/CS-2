...
/* Preparo LEDs para la primera interrupción*/
/* guia bits           10987654321098765432109876543210*/
	mov	r3, #0b00000000000000000000001000000000			@ para encender GPIO 9 luego en la RTI
	mov	r4, #0b00001000000000000000000000000000			@ para apagar GPIO 27 luego en la RTI
...
orden al puerto que enciende los LEDs o al puerto que los apaga, respectivamente:*/
	...
	str r1, [r0] 							@ Escribo variable en la posición indicada por puntero
        ...
        streq   r3, [r0, #GPSET0] 					@ enciendo/almaceno si flag Z=1 (activo)
	strne	r3, [r0, #GPCLR0]					@ apago/almaceno si flag Z=0 (inactivo)
	...
	ldr r2, =1000000 						@ medio segundo a 1 Hz (tiempo entre interrupción e interrupción)
...
red2:
	mov	r3, #0b00000000000000000000010000000000
	b	vuelta
yellow1:
	mov	r3, #0b00000000000000000000100000000000
	b	vuelta
yellow2:
	mov	r3, #0b00000000000000100000000000000000
	b	vuelta
green1:
	mov	r3, #0b00000000010000000000000000000000
	b	vuelta	
green2:
/* guia bits           10987654321098765432109876543210*/
	mov	r3, #0b00001000000000000000000000000000
	b	vuelta
red1:
/* guia bits           10987654321098765432109876543210*/
...
ledst: .word 1								@ en memoria esa variable cuyo valor es 0
